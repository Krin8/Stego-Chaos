...
class LitUnsupervisedSegmenter(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Step the cluster probe warmup scheduler
        cluster_sched = self.lr_schedulers()
        if cluster_sched is not None:
            cluster_sched.step()
        ...
        if self.cfg.reset_probe_steps is not None and self.global_step == self.cfg.reset_probe_steps:
            ...
            self.trainer.optimizers[2] = torch.optim.Adam(list(self.cluster_probe.parameters()), lr=5e-4)
